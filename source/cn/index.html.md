...
| code | 0     |
| msg  | "suc" | code>0失败 |
| data | 如下    | <br>下单返回：订单id，状态码，成功(0)/失败(1)<br> <br>撤单返回：订单id，状态码，成功(0)/失败(1)<br>|
...
### 心跳消息
当用户的Websocket客户端连接到WBF的Websocket服务器后，服务器会定期（当前设为5秒）向其发送ping消息并包含一整数值如下：
```javascript
{"ping": 1595935723334}
```
...
```javascript
{"event": "sub", "params":{"channel":"channel name", "cb_id":"id generated by client"}}
```
### 取消订阅
